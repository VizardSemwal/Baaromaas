...
<Pricing 
data={Point == "Base"?Base:Point == "Haridwar"?Haridwar:Delhi}  
price={Price}
...
var options = {
  ...
  amount: (((Price * person) * 0.3).toFixed(0)) * 100,
  name: Name,
  prefill: {
    email: email,
    contact: `+91 ${Mobile}`,
...
RazorpayCheckout.open(options).then((data) => {
    ...
    selected:Point,
    email:email,
    Fullpayment:(Price * person) - (((Price * person) * 0.3).toFixed(0)),
    BookingAmount:(((Price * person) * 0.3).toFixed(0))
...
>
    ...
    <Text style={{fontWeight:"bold",fontSize:18}}>   ₹ {((Price * person) * 0.3).toFixed(0)}</Text></Button>
